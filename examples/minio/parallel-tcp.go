...
func makeNewClients(hostFile, accessKey, secretKey string) ([]*minio.Client, error) {
	hosts := readHosts(hostFile)
	minioClients := make([]*minio.Client, 0, len(hosts))
...
func getCore(key string, clients []*minio.Client) *minio.Core {
	return &minio.Core{clients[crcHashMod(key, len(clients))]}
}

func getClient(key string, clients []*minio.Client) *minio.Client {
	...
	for i := 0; i < concurrent; i++ {
		go func(i int) {
			...
			for j := 0; j < objectsPerWorker; j++ {
				...
				var op Operation
				op.Start = time.Now()
				info, perr := getClient(objName, clients).PutObject(context.Background(), "warp-benchmark-bucket",
					objName, bytes.NewReader(buf), int64(len(buf)), popts)
				op.End = time.Now()
				if perr != nil {
					log.Println(perr)
				}
	...
	for i := 0; i < concurrent; i++ {
		go func(i int) {
			...
			for j := 0; j < objectsPerWorker; j++ {
				...
				var op Operation
				op.Start = time.Now()
				obj, info, _, perr := getCore(objName, clients).GetObject(context.Background(), "warp-benchmark-bucket", objName, gopts)
